...
def matrix_divided(matrix, div):
    ...
    if (not isinstance(matrix, list) or matrix == [] or
            ...
            not all((isinstance(elems, int) or isinstance(elems, float))
                    for elems in [num for rows in matrix for num in rows])):
        raise TypeError("matrix must be a matrix (list of lists) of "
                        "integers/floats")

    if not all(len(rows) == len(matrix[0]) for rows in matrix):
        raise TypeError("Each rows of the matrix must have the same size")
